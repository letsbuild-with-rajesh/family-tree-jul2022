...
const MemberDetailsPopper = (props) => {
	...
	const submitHandler = async (e) => {
		const isValid = name && gender && dob && (addSpouseClicked ? spouseName && spouseGender : true);
		if (isValid) {
			e.preventDefault();
			if (type === 'add') {
				const payload = {
		...
		<div className={styles.overlayContainer}>
			<div className={styles.popperContainer}>
				...
				<form>
					...
					<div>
						<button type="button" onClick={() => setAddSpouseClicked(add => !add)}>{`${addSpouseClicked ? 'Remove' : 'Add'} Spouse`}</button>
					</div>
					{
						addSpouseClicked && (
							<>
					...
					<div className={styles.submitBtn}>
						...
						<button type="button" onClick={resetFormAndClose}>Cancel</button>
					</div>
				</form>
			</div>
		</div>
